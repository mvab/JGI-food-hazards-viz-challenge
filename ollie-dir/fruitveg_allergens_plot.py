...
food_haz = pd.read_csv('data/tidy/food_hazards_data_allergens_tidy.csv')
...
counts = pd.DataFrame(columns=['origin', 'allergen', 'count', 'freq'])
for tup, df in food_haz.groupby(['origin_country_EU', 'hazard_type_large']):
...
diff_df = pd.DataFrame(columns=['allergen', 'freq_diff'])
for haz, df in counts.groupby('allergen'):
    if len(df) == 1:
...
sns.barplot('freq_diff', 'allergen', data=diff_df, palette=custom_palette)
plt.ylabel('')
plt.xlabel('Relative frequency change after switching to non-EU imports')
plt.title('Allergen contaminants in fruits and vegetables: EU vs non-EU imports')
plt.tight_layout()
plt.savefig('allergens_eu_noneu.png')
